...
const analyzeRules = ({ sku, today, yesterday, metrics, latestAction, previousAction, records, periodComparison }) => {
  ...
  const periodSkuRow = periodComparison?.skuRows?.find?.((row) => row.sku === sku);
  if (periodComparison?.hasComparison && periodSkuRow) {
    if (periodSkuRow.adSpendDelta < 0 && periodSkuRow.currentOrders >= periodSkuRow.previousOrders * 0.95 && periodSkuRow.profitDelta > 0) recommendations.push(makeRecommendation('保持当前策略', `${sku} 当前选择时间段较上期广告费下降、订单稳定、利润提升，建议保持当前预算策略。`, '高'));
    if (periodSkuRow.adSpendDelta > Math.max(50, periodSkuRow.previousAdSpend * 0.25) && periodSkuRow.currentOrders <= periodSkuRow.previousOrders * 1.1 && periodSkuRow.profitDelta <= 0) recommendations.push(makeRecommendation('控制广告花费', `${sku} 当前选择时间段广告费明显上升，但订单和利润没有同步增长，建议降低预算或出价。`, '高'));
    if (periodSkuRow.currentOrders > periodSkuRow.previousOrders && periodSkuRow.profitDelta > 0 && periodSkuRow.currentRoi > 2 && periodSkuRow.currentStock >= 30) recommendations.push(makeRecommendation('加大预算', `${sku} 当前选择时间段订单增长、利润增长且 ROI ${periodSkuRow.currentRoi.toFixed(2)}，库存充足，可适当加预算。`, '中'));
    if (periodSkuRow.profitDelta < 0 && periodSkuRow.currentProfit < periodSkuRow.previousProfit) recommendations.push(makeRecommendation('控制广告花费', `${sku} 当前选择时间段利润较上期下降，建议控制广告费并排查低 ROI 广告位置。`, '高'));
    if (periodSkuRow.currentStock > 0 && periodSkuRow.currentStock < 30) recommendations.push(makeRecommendation('补货', `${sku} 当前选择时间段库存低于 30，不建议加大广告，优先补货。`, '高'));
  }
...
export const buildEffectAnalysis = (records = [], actions = [], filters = {}, periodComparison = null) => {
  ...
  const analyses = [...bySku.entries()].map(([sku, skuRecords]) => {
    ...
    const targetDate = filters.date || filters.endDate || sorted.at(-1)?.date;
    const today = sorted.filter((record) => record.date <= targetDate).at(-1) || (!filters.date && !filters.endDate ? sorted.at(-1) : null);
    ...
    const ruleResult = analyzeRules({ sku, today: enrichRecord(today), yesterday: enrichRecord(yesterday), metrics, latestAction, previousAction, records: sorted.slice(0, todayIndex + 1), periodComparison });
